    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        for row_idx in 0..num_rows {
            ...
            for elem_idx in start..end {
                match eval_bool(&tree, struct_array, &struct_fields, elem_idx)? {
                    Some(true) => {
                        any_match = true;
                        break;
                    }
...
fn eval_bool(
    ...
    match op {
        "AND" => {
            for a in args {
                match eval_bool(a, struct_array, struct_fields, elem_idx)? {
        ...
        "NOT" => {
            ...
            Ok(eval_bool(&args[0], struct_array, struct_fields, elem_idx)?.map(|b| !b))
        }
        ">" | ">=" | "<" | "<=" | "=" | "!=" => {
            if args.len() != 2 {
